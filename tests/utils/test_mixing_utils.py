...
from transformers.utils.mixing_utils import mix
...
test_cases = [
    ...
    ("mistral mlp query mix", True, ["mlp", "q_proj"],
        MistralForCausalLM, MistralConfig(
            **simple_arch
        )),
    ("mistral all mix", True, ["mlp", "q_proj", "k_proj", "v_proj"],
        MistralForCausalLM, MistralConfig(
            **simple_arch
        )),
    ...
    ("Llama all mix", True, ["mlp", "q_proj", "k_proj", "v_proj"],
        LlamaForCausalLM, LlamaConfig(
            **simple_arch
        )),
]
...
if __name__ == "__main__":
    ...
    for idx, (name, cpu_ok, modules_to_mix, cls, config) in enumerate(test_cases):
        ...
        try :
            ...
            gc.collect()
            logging.info("mixing...")
            m, info = mix(TMP_DIR, [TMP_DIR]*4, modules_to_mix)
