...
from transformers import MoeLlamaConfig, MoeLlamaForCausalLM
...
configs = [
    ...
    ("MoeLLaMa with MLP MOE always-on expert", True,
        MoeLlamaConfig(
    ...
    ("MoeLLaMa with ALL MOE with flash attn", False,
        MoeLlamaConfig(
            num_experts_per_tok=2,
            num_local_experts=4,
            moe_query=True,
    ...
    for idx, (name, cpu_ok, config) in enumerate(configs):
        ...
        try :
            ...
            if not torch.cuda.is_available() and not cpu_ok:
                ...
                continue
            simpleforward(MoeLlamaForCausalLM, dtype, device)
